...
namespace Shared.Core.View.Scene
{
    ...
    public class UIScene : IoCMonoBehavior, IUIScene, IUIBackKeyHandler, ISharedUtility
    {
        ...
        [Header("Dialog Sector")]
        [SerializeField] private RectTransform dialogParent;
        ...
        [Header("Loading Sector")]
        [SerializeField] private RectTransform loadingParent;
        ...
        [Header("Floating Message Sector")]
        [SerializeField] private RectTransform floatingMessageParent;
        ...
        [Header("Halo Sector")] 
        [SerializeField] private RectTransform haloParent;
        ...
        public IAsyncOperation<T> ShowDialog<T>(DialogAction dialogAction = DialogAction.HideLastOne) where T : MonoBehaviour, IUIDialog
        {
            ...
            if (newDialog == null)
            {
                ...
                if (newDialog is IUISingleton) _dialogCache.Upsert(fullName, newDialog);
            }
            newDialog.transform.SetAsLastSibling();
            if (newDialog is IUISingleton) _dialogStack.Remove(newDialog);
        ...
        private IEnumerator _PresentShowDialog(MonoBehaviour oldOne, MonoBehaviour newOne)
        {
            ...
            if (oldOne != null)
            {
                ...
                if (oldOne is not IUISingleton) Destroy(oldOne.gameObject);
        ...
        public void HideHalo(MonoBehaviour monoBehaviour)
        {
            var fullName = monoBehaviour.GetType().FullName;
            if (string.IsNullOrEmpty(fullName))
                throw new ArgumentException($"string.IsNullOrEmpty(fullName) for {monoBehaviour.GetType()}");
